...
import { usePrisma, useLogger, usePrismaClientKnownRequestError } from "../../shared-clients";
			...
			await prisma.$transaction(async (tx) => {
				...
				const chatsAdded = (
					await tx.chat.createMany({
						data: chats.filter((c) => !existingIds.includes(c.id)).map((c) => ({ ...transformPrisma(c), sessionId })),
					})
				).count;
		...
		for (const update of updates) {
			try {
				...
				await prisma.chat.update({
					...
					data: {
						...data,
						unreadCount: typeof data.unreadCount === "number" ? (data.unreadCount > 0 ? { increment: data.unreadCount } : { set: data.unreadCount }) : undefined,
					},
